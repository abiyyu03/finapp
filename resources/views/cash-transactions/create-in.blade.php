...
@section('breadcrumb')
    ...
    <span class="text-slate-700">Cash In</span>
@endsection

@section('content')
    <div class="mx-auto max-w-xl py-6">
        <h1 class="text-lg font-semibold text-slate-900">Cash In</h1>
        <p class="mt-1 text-sm text-slate-500">Money received. Generates: Debit the cash account, Credit the counter account.</p>

        <form method="POST" action="{{ route('cash-transactions.store-in') }}" class="mt-4 space-y-4 rounded-lg border border-slate-200 bg-white p-6 shadow-sm">
            @csrf
