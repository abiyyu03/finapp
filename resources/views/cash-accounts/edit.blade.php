...
@section('breadcrumb')
    ...
    <span class="text-slate-700">{{ $cashAccount->name }}</span>
@endsection

@section('content')
    <div class="mx-auto max-w-2xl py-6">
        <h1 class="text-lg font-semibold text-slate-900">Edit {{ $cashAccount->name }}</h1>

        <form method="POST" action="{{ route('cash-accounts.update', $cashAccount) }}" class="mt-4 space-y-6 rounded-lg border border-slate-200 bg-white p-6 shadow-sm">
            @csrf
            @method('PUT')
